...
class MenuPokjaaPieList extends StatefulWidget {
  ...
  @override
  State<MenuPokjaaPieList> createState() => _MenuPokjaaPieListState();
}

class _MenuPokjaaPieListState extends State<MenuPokjaaPieList> {
  @override
  void initState() {
    super.initState();
    getData();
  ...
  void getData() async {
    try {
      List<DataPokjaModel> newData = await _fetchDataRekapPieChart();
  ...
  Future<List<DataPokjaModel>> _fetchDataRekapPieChart() async {
    final url = '${baseUrl}data-pokjaa-view';
    try {
                          ...
                          child: UnExpandableRecords(
                            data: SettingData(
                                ...
                                onTap: () {
                                  ...
                                  if (index == 0) {
                                    Get.to(() => SuperDataPokjaaPieScreen(
                                          title:
                                              'Perbandingan Total ${listName[0]} dan Total ${listName[1]}',
...
class SuperDataPokjaaPieScreen extends StatefulWidget {
  final String title;
  final double totalData1;
  final double totalData2;
  final List<String> name;

  const SuperDataPokjaaPieScreen(
      {super.key,
  ...
  @override
  _SuperDataPokjaaPieScreenState createState() =>
      _SuperDataPokjaaPieScreenState();
}

class _SuperDataPokjaaPieScreenState extends State<SuperDataPokjaaPieScreen> {
  RxBool isLoading = true.obs;
