...
import '../controller/data_pokjac_input_controller.dart';
import '../model/data_pokjac_model.dart';

class DataPokjacInputScreen extends StatefulWidget {
  DataPokjacInputScreen({
    super.key,
  });

  @override
  State<DataPokjacInputScreen> createState() => _DataPokjacInputScreenState();
}

class _DataPokjacInputScreenState extends State<DataPokjacInputScreen> {
  final controller = Get.put(DataPokjacInputController());
  @override
  void dispose() {
...
class _StepperUmumValue extends StatelessWidget {
  ...
  final controller = Get.find<DataPokjacInputController>();
  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
...
class _StepperKeluarga extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: List.generate(
        controller.datas.length - 14, // Hanya indeks 20 sampai 44 ini kn 24 14
        (index) => WidgetForm(
          hint: 'Masukkan ${controller.datas[index + 14]}',
          tittle: controller.datas[index + 14],
          controller: controller.editingControllers[index + 14],
          errorText: '',
          icon: Remix.pencil_line,
          inputFormatters: [controller.keyboardTypes[index + 14]],
        ),
      ),
    );
  }
